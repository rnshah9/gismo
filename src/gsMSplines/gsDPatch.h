...
class gsDPatch  //: public gsMappedGeom<d,T>
{
    protected:
    ...
    public:
        ...
        /// Empty constructor
        gsDPatch() : m_patches(gsMultiPatch<T>())
        { }
         ...
         * @param      mp    Multipatch of the geometry
         */
        gsDPatch(gsMultiPatch<T> const & mp, const std::vector<patchCorner> & C0_corners) ;


        /// Copy constructor
        gsDPatch( const gsDPatch& other );
    ...
    protected:
         ...
         */
        const index_t _indexFromVert(gsMultiBasis<T> bases, index_t index, const patchCorner corner, const patchSide side, index_t offset = 0, index_t levelOffset = 0);
        const index_t _indexFromVert(index_t index, const patchCorner corner, const patchSide side, index_t offset = 0, index_t levelOffset = 0);
         ...
         */
        const std::vector<index_t> _indexFromVert(gsMultiBasis<T> bases, std::vector<index_t> index, const patchCorner corner, const patchSide side, index_t offset = 0, index_t levelOffset = 0);
        const std::vector<index_t> _indexFromVert(std::vector<index_t> index, const patchCorner corner, const patchSide side, index_t offset = 0, index_t levelOffset = 0);
